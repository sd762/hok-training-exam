...
支援列印；共5個分頁，考測結果分析把「不及格」拆成「測驗未達標」跟「考試紀律違規」（監考自動中止）
兩種原因，應考次數分析看考幾次才有結果的分布（1/2/3/4次以上）。圖表形式依任務決定：組成佔比（學員/
年齡分布）選定單一機構時用甜甜圈圖，結果狀態類的（考測結果/應考次數）因為分類較多（6種），一律用
柱狀圖，不用甜甜圈圖（甜甜圈色票只驗證過3色安全上限，見 dataviz 技能 palette.md）。
